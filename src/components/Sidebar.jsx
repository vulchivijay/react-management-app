      ...
      <ul className="mt-6">
        {showProjects && showProjects.project.map((project, index) => {
          return <li key={index} className="my-1">
            <button onClick={() => viewProjectDetails(project.id)} className="w-full text-left px-2 py-1 rounded-sm text-stone-400 hover:text-stone-200 hover:bg-tone-800">{project.title}</button>
          </li>
        })}
      </ul>
    </aside>
